...
describe("Calling Panel", () => {
  new Array(1500).fill("-").forEach((item, index) => {
    it(`lead ${index + 1}`, () => {
      cy.session(
        "laravel",
        () => {
      ...
      // cy.visit("https://devcrms.tractorfirst.com");
      // cy.get(":nth-child(2) > .form-control").type("sweta@tj.com");
      // cy.get(":nth-child(3) > .form-control").type("pass@123");
      // cy.get(".btn").click();
      // cy.wait(2000);
      ...
      // cy.get("[id^=brand]").select(randomNumber(86));
      // cy.wait(2000);
      // cy.get("[id$=product_list]").then((list) => {
      //   if (list[0].length === 1) {
      //     cy.get("#truck_product_list").select(0);
      //   } else {
      //     cy.get("[id$=product_list]").select(
      ...
      //   }
      // });
      // cy.get(".card-body > :nth-child(1) > .col-sm-6 > .form-control").type(
      //   generateNumbers(10000, 50000)
      // );
      // cy.get(
